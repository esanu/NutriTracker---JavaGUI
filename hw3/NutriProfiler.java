...
public class NutriProfiler {
	...
	static void createNutriProfile(Person person) {
		person.recommendedNutrientsList.clear();	//clear list each time this method is invoked
		float energyReq = person.calculateEnergyRequirement();	//calculated the energy requirement..
		RecommendedNutrient recommendedEnergy = new RecommendedNutrient(ENERGY_NUTRIENT_CODE , energyReq);	//passes as parameter to create recommended nutrients object
		person.recommendedNutrientsList.add(recommendedEnergy);	//adds to recommendednutrientslist
		
		float [] nutriQuantity = person.calculateNutriRequirement();	//creates an array populated with array returned in person method
		for(int i = 0; i < nutriQuantity.length; i++) {
			float quant = nutriQuantity[i];		
			String nutrient = NutriEnum.values()[i].getNutrientCode();	//gets nutrient code from enum for each value
			RecommendedNutrient recommendedNutrients = new RecommendedNutrient(nutrient, quant);	//passes nutrient and quantity to recommended nutrient object
			person.recommendedNutrientsList.add(recommendedNutrients);	//adds nutrient to list
		}	
	}
}

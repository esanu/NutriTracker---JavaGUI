...
public class NutriByte extends Application{
	...
	static Stage stage;					//made static to make accessible in the controller
...
//				view.ageTextField.setStyle("-fx-text-inner-color: black;");
				if(Float.parseFloat(newValue) < 0) view.ageTextField.setStyle("-fx-text-inner-color: red;");
				else view.ageTextField.setStyle("-fx-text-inner-color: black;");
			}
...
//				view.weightTextField.setStyle("-fx-text-inner-color: black;");
				if(Float.parseFloat(newValue)< 0) view.weightTextField.setStyle("-fx-text-inner-color: red;");
				else view.weightTextField.setStyle("-fx-text-inner-color: black;");
			}
			catch(NumberFormatException e){
...
//				view.heightTextField.setStyle("-fx-text-inner-color: black;");
				if(Float.parseFloat(newValue) < 0) view.heightTextField.setStyle("-fx-text-inner-color: red;");
				else view.heightTextField.setStyle("-fx-text-inner-color: black;");
			}
			catch(NumberFormatException e){
	...
	Callback<CellDataFeatures<RecommendedNutrient, String>, ObservableValue<String>> recommendedNutrientQuantityCallback = new Callback<CellDataFeatures<RecommendedNutrient, String>, ObservableValue<String>>() {
		@Override
		public ObservableValue<String> call(CellDataFeatures<RecommendedNutrient, String> arg0) {
			for(RecommendedNutrient nutrient: person.recommendedNutrientsList) {		//for each recommended nutrient...
				if(nutrient.getNutrientCode().equals(arg0.getValue().getNutrientCode())) {		//..if there's a match between the nutrient code in list and in recommended nutrient object...
					return (new SimpleStringProperty(String.format("%.2f", nutrient.getNutrientQuantity()))); 	//..return the recommended nutrient quantity
				}
			}
	...
	Callback<CellDataFeatures<RecommendedNutrient, String>, ObservableValue<String>> recommendedNutrientUomCallback = new Callback<CellDataFeatures<RecommendedNutrient, String>, ObservableValue<String>>() {
		@Override
		public ObservableValue<String> call(CellDataFeatures<RecommendedNutrient, String> arg0) {
			//returns nutrient and assigns to a Nutrient object when there's a match in nutrientcode in the nutrientsmap and recommendednutrients
			Nutrient nutrient = Model.nutrientsMap.get(arg0.getValue().getNutrientCode());	
			return nutrient.getNutrientUomProperty();		//returns nutrientUom as a property
		}
	};	
}
